...
from cocotbext.axi import AxiStreamFrame, AxiStreamSink, AxiStreamSource, AxiStreamBus
...
async def test_fips_vector(dut):
  ...
  await axis_source.send(AxiStreamFrame(plaintext))
...
async def test_encryption(dut, test_num):
  cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
  ...
  for _ in range(random.randint(MIN_FRAME_NUM, MAX_FRAME_NUM)):
    ...
    expected = b"".join(cipher.encrypt(payload[j:j+16]) for j in range(0, len(payload), 16))
